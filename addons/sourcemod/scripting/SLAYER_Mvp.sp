...
#define MAX_MVP_COUNT 500 // Maximum Numbers of Mvps
...
public void OnClientPostAdminCheck(int client)
{
	...
	{
		OnClientCookiesCached(client); // Load Client Cookies
	}
}
...
public Action Event_RoundMVP(Handle event, const char[] name, bool dontBroadcast)
{
	if(sEnable.BoolValue)
	{
		...
		{
			int mvp;
			if(StrEqual(MVPPlaylist[client], "") || StrEqual(MVPPlaylist[client], " ")) // If No Playlist Selected
			{
				char random_mvp[200];
				Format(random_mvp, sizeof(random_mvp), "%t", "Random MVP");
				if(IsFakeClient(client) && sBotMvp.BoolValue)strcopy(NameMVP[client], sizeof(NameMVP[]), random_mvp);
				if(StrEqual(NameMVP[client], random_mvp) || SelectedMvp[client] == -1) // If Random MVP is Selected
				{
					...
					} while(!CanUseMVP(client, mvp)); // If client don't have access to that random MVP then Select another random mvp again
				...
				else {mvp = SelectedMvp[client];} // If Random MVP is not Selceted
			...
			else // If Playlist Selected
			{
				...
				int playlist_id = FindMVPPlaylistIDByName(client, MVPPlaylist[client]); 
				// Now we Get MVP ID by Giving Specific MVP Name from Player's Selected Playlist
				mvp = FindMVPIDByName(MVPPlaylistMVPName[client][playlist_id][PlayedMVPFromPlaylistCount[client][playlist_id]]); 
				// Increasing Counter by 1
				PlayedMVPFromPlaylistCount[client][playlist_id]++; 
				// If 'PlayedMVPFromPlaylistCount' Counter becomes Equal to Total MVPs in Player's Selected Playlist 
				if(PlayedMVPFromPlaylistCount[client][playlist_id] >= MVPPlaylistMVPCount[client][playlist_id])
				{
					PlayedMVPFromPlaylistCount[client][playlist_id] = 0; // then Reset Counter
				}
			...
			for(int i = 1; i <= MaxClients; i++)
			{
				...
				{
					// Announce MVP
					if(sMessage.IntValue == 2 || sMessage.IntValue == 3)PrintHintText(i, "%t", "MVP_Hint", client, MVPName[mvp]);
					if(sMessage.IntValue == 1 || sMessage.IntValue == 3)CPrintToChat(i, "%t %t", "Tag","MVP_Chat", client, MVPName[mvp]);
...
void AddSoundsInConfig(char[] SoundFolderPath, int TotalSoundsToLoad=0, char[] SoundsNamePrefix="", char[] PremiumSoundsFlag="")
{
	...
	while (dL.GetNext(fileBuffer[fileCounter], sizeof(fileBuffer))) 
	{
		...
		else fileCounter++; // Otherwise Continue Counting
	}
	fileCounter--; // One Minus Because Counter Count extra one in the end of loop
...
bool CanUseMVP(int client, int id)
{
	...
	{
		if(IsFakeClient(client) && !sAllowBot.BoolValue)return false; // If client is bot and not allow to use premium mvps then return false
		else if(IsFakeClient(client) && sAllowBot.BoolValue)return true; // if client is bot and allow to use premium mvps then return true
		if(CanUsePremiumMVP(client, id))return true; // If client is real player and can use PremiumMVP
...
stock bool CanUsePremiumMVP(int client, int id)
{
	...
	else if(!StrEqual(MVPFlag[id], "") && !StrEqual(MVPFlag[id], " ")) // If there is any admin flag restriction then check client have those flags or not
	{
